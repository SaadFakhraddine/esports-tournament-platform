import type { BracketMatch, BracketStructure, BracketTransition, Slot, Team } from './types'

/**
 * Generates a single elimination bracket
 * Teams are paired based on seeds: 1 vs lowest, 2 vs 2nd lowest, etc.
 */
export function generateSingleElimination(teams: Team[]): BracketStructure {
 ...
 */
export function generateRoundRobin(teams: Team[]): BracketStructure {
  const sortedTeams = [...teams].sort((a, b) => {
    const seedA = a.seed || 999
    const seedB = b.seed || 999
    return seedA - seedB
 ...
 */
export function generateDoubleElimination(teams: Team[]): BracketStructure {
  const sortedTeams = [...teams].sort((a, b) => {
    const seedA = a.seed || 999
    const seedB = b.seed || 999
    return seedA - seedB
 ...
 */
export function generateSwiss(teams: Team[]): BracketStructure {
