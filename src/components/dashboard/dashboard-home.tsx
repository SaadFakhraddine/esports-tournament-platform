...
type TournamentOutputs = inferRouterOutputs<AppRouter>['tournament']
type TeamOutputs = inferRouterOutputs<AppRouter>['team']
...
export interface DashboardHomeProps {
  session: Session
  stats: UserOutputs['getDashboardStats']
  tournaments: TournamentOutputs['getParticipatingTournaments']
  teams: TeamOutputs['getMyTeams']
  recentActivity: DashboardActivityItem[]
}

export function DashboardHome({ session, stats, tournaments, teams, recentActivity }: DashboardHomeProps) {
    ...
    <div className='space-y-8'>
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground mt-2'>Here&apos;s what&apos;s happening with your tournaments and teams</p>
        </div>
        {isOrganizer && (
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        <StatCard
          ...
          value={stats.activeTournamentsCount.toString()}
        ...
        <StatCard
          ...
          description={`Member of ${stats.teamsCount} team${stats.teamsCount !== 1 ? 's' : ''}`}
          icon={<Users className='h-4 w-4 text-muted-foreground' />}
        />
        <StatCard
          title='Upcoming Matches'
          value={stats.upcomingMatchesCount.toString()}
        ...
        <StatCard
          title='Win Rate'
          value={`${stats.winRate}%`}
          description={`${stats.wonMatches} of ${stats.totalMatches} matches won`}
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-7'>
        <Card className='col-span-4'>
          <CardHeader>
            ...
            <CardDescription>Tournaments you&apos;re participating in or organizing</CardDescription>
          </CardHeader>
          <CardContent>
            {tournaments.length > 0 ? (
              ...
              <div className='text-center py-8'>
                <p className='text-sm text-muted-foreground'>No tournaments yet. Browse tournaments to join!</p>
              </div>
            )}
          ...
          <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
            ...
            <div className='border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center text-center hover:border-primary transition-colors cursor-pointer'>
              <Link href='/teams/create' className='w-full'>
                ...
                <p className='text-xs text-muted-foreground mt-1'>Start building your roster</p>
              </Link>
            </div>
          </div>
        </CardContent>
    ...
    >
      ...
      <div className='flex-1 min-w-0'>
        ...
        {activity.metadata?.tournamentName && (
          <p className='text-xs text-muted-foreground mt-1'>in {activity.metadata.tournamentName}</p>
        )}
      </div>
      <span className='text-xs text-muted-foreground whitespace-nowrap'>{getTimeAgo(activity.timestamp)}</span>
    </div>
  )
